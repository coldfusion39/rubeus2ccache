...
def main():
    ...
    parser.add_argument('inputfile', help='Rubeus dump output file')
    parser.add_argument('-e', '--expired', help='Process expired tickets', action='store_true', default=False, required=False)
    args = parser.parse_args()

    msg = logging.msg()

    if os.path.exists(args.inputfile) is False:
    ...
    while line:
        ...
        if newticket:
            # We are at the beginning of the base64 data. Continue past the new line
            if len(line.strip()) == 0 and inticket is False:
                continue

            # First line of the ticket data. process the data
            elif len(line.strip()) > 0 and inticket is False:
                inticket = True
                ticketdata += line.strip()

            # In the body of the ticket data. process the data
            elif len(line.strip()) > 0 and inticket is True:
                ticketdata += line.strip()
